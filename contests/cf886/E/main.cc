...
i64 bsearch(i64 cardboard, vec<i64> &v) {
    i64 left = 0, right = 1e9;
    while (left <= right) {
        i64 mid = (left + right) / 2;

        i64 computed = compute(mid, v);
        if (computed < cardboard) {
            left = mid + 1;
        } else if (computed > cardboard) {
            right = mid - 1;
        } else {
            return mid;
        }
