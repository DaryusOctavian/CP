...
void dfs(i64 depth, i64 x, i64 y, vec<vec<char>> &v, i64 &n, i64 &m) {
  if (depth > 4) {
    return;
  }
  rs++;
  ...
  v[x][y] = '.';
  if (x + 1 < n && v[x + 1][y] == '*') {
    dfs(depth + 1, x + 1, y, v, n, m);
  }
  if (y + 1 < m && v[x][y + 1] == '*') {
    dfs(depth + 1, x, y + 1, v, n, m);
  }
  if (x - 1 >= 0 && v[x - 1][y] == '*') {
    dfs(depth + 1, x - 1, y, v, n, m);
  }
