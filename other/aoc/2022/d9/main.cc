...
vi64 posx = {1, -1, 0, 0, 1, -1, 1, -1, 0};
vi64 posy = {0, 0, 1, -1, 1, -1, -1, 1, 0};

bool inrange(p64 a, p64 b) {
  for (i64 i : range(9)) {
    p64 crt = {a.ft + posx[i], a.sd + posy[i]};
    if (crt == b) {
      return true;
    }
  }
...
void move(char dir, p64 &h, p64 &t, set<i64> &vis) {
  p64 old = h;
  vis.insert(t.ft * 1e9 + t.sd);
