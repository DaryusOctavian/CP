...
vec<short> posx = {1, -1, 0, 0};
vec<short> posy = {0, 0, 1, -1};

i64 bfs(i64 weight, vec<vec<short>> &v) {
  dp64 q = {{0, 0}};
  vec<vec<short>> vis(v.size(), vec<short>(v.size(), -1));
  vis[0][0] = v[0][0] < weight ? 1 : 0;

  while (!q.empty()) {
    p64 crt = popf(q);
    for (i64 i : range(4)) {
      p64 next = {crt.ft + posx[i], crt.sd + posy[i]};
      if (next.ft < 0 || next.ft >= v.size()) {
        continue;
      }
      if (next.sd < 0 || next.sd >= v.size()) {
        continue;
      }

      if (vis[next.ft][next.sd] == -1) {
        if (weight > v[next.ft][next.sd]) {
          vis[next.ft][next.sd] = vis[crt.ft][crt.sd] + 1;
          q.push_back({next.ft, next.sd});
        } else {
          vis[next.ft][next.sd] = vis[crt.ft][crt.sd];
          q.push_front({next.ft, next.sd});
        }
      }
    }
  }
