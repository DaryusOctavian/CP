...
i64 bfs(p64 pos, vec<vp64> &v, i64 gr) {
  deq<p64> q;
  i64 res = 0;

  q.push_back(pos);
  res++;
  v[pos.ft][pos.sd].ft = gr;
  ...
  while (!q.empty()) {
    ...
    for (i64 k : range(4)) {
      ...
      if (x >= 0 && x < n && y >= 0 && y < n && !v[x][y].ft &&
          v[p.ft][p.sd].sd == v[x][y].sd) {
        q.push_back({x, y});
        res++;
        v[x][y].ft = gr;
      }
...
i64 truebfs(p64 pos, vec<vp64> &v) {
  vec<vb> vis(n, vb(n, false));
  deq<p64> q;
  i64 res = 0;

  q.push_back(pos);
  res++;
  vis[pos.ft][pos.sd] = true;
...
vi64 rs(7, 0);
i64 adjbfs(p64 pos, vec<vp64> &v) {
  rs.assign(7, 0);
  deq<p64> q;

  vec<vb> checked(n, vb(n, false));
  q.push_back(pos);
  v[pos.ft][pos.sd].ft = 0;
  ...
  while (!q.empty()) {
    ...
    for (i64 k : range(4)) {
      ...
      if (x >= 0 && x < n && y >= 0 && y < n && v[x][y].ft) {
        if (v[p.ft][p.sd].sd == v[x][y].sd) {
          q.push_back({x, y});
          v[x][y].ft = 0;
        } else {
          if (!checked[x][y]) {
            rs[v[x][y].sd] += truebfs({x, y}, v);
            checked[x][y] = true;
          }
        }
      }
...
void flood(p64 pos, vec<vp64> &v, i64 a, i64 b) {
  vec<vb> vis(n, vb(n, false));
  deq<p64> q;

  q.push_back(pos);
  v[pos.ft][pos.sd].sd = a;
  vis[pos.ft][pos.sd] = true;
  ...
  while (tc--) {
    ...
    for (i64 i : range(n)) {
      for (i64 j : range(n)) {
        if (!v[i][j].ft) {
          gr++;
          groups.push_back({bfs({i, j}, v, gr), gr});
        }
      }
    }
      ...
      for (i64 j : range(n)) {
        if (v[i][j].ft == groups[strt].sd) {
          ...
          do {
            ...
            i64 crt = adjbfs({i, j}, v);
            resv[crt]++;
            flood({i, j}, v, crt, v[i][j].sd);
          } while (!ok);
          break;
        }
      }
