...
bool solve(i64 x, i64 y, set<p64> &used, set<p64> &walls) {
  str ans;
  used.insert({x, y});

  if (used.find({x + 1, y}) == used.end() &&
      walls.find({x + 1, y}) == walls.end()) {
    cout << "down" << endl;
    cin >> ans;
    if (ans == "ok") {
      if (solve(x + 1, y, used, walls)) {
        return true;
      } else {
        cout << "up" << endl;
        cin >> ans;
  ...
  if (used.find({x, y + 1}) == used.end() &&
      walls.find({x, y + 1}) == walls.end()) {
    cout << "right" << endl;
    cin >> ans;
    if (ans == "ok") {
      if (solve(x, y + 1, used, walls)) {
        return true;
      } else {
        cout << "left" << endl;
        cin >> ans;
