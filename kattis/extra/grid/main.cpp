...
void bfs(i64 x, i64 y, vec<vec<char>> &v, vec<vec<bool>> &vvis, vec<vi64> &d) {
  deque<p64> q;
  q.push_back({x, y});
  vvis[x][y] = true;
  ...
  while (!q.empty()) {
    ...
    y = p.sd;
    for (i64 i : range(4)) {
      i64 xx = x + dirx[i] * (v[x][y] - '0');
      i64 yy = y + diry[i] * (v[x][y] - '0');
      if (xx >= 0 && xx < n && yy >= 0 && yy < m && !vvis[xx][yy]) {
        vvis[xx][yy] = true;
        q.push_back({xx, yy});
...
int main() {
  ...
  vec<vec<bool>> vvis(n, vec<bool>(m, false));
  vec<vi64> d(n, vi64(m, INF));
  for (i64 i : range(n)) {
    for (i64 j : range(m)) {
      cin >> v[i][j];
    }
