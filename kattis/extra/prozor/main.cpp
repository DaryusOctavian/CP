...
int main() {
  ...
  for (i64 i = 1; i <= n - diag - 1; i++) {
    for (i64 j = 1; j <= m - diag - 1; j++) {
      i64 s = prefix[i + diag][j + diag] - prefix[i][j + diag] -
              prefix[i + diag][j] + prefix[i][j];
      if (s > mx) {
        mx = s;
        res = {i - 1, j - 1};
      }
  ...
  diag += 1;
  a[res.ft][res.sd] = '+';
  a[res.ft + diag][res.sd + diag] = '+';
  a[res.ft + diag][res.sd] = '+';
  a[res.ft][res.sd + diag] = '+';
  i64 i = res.ft, j = res.sd;
  do {
    a[i][j] = a[i][j] == '+' ? '+' : '-';
    j++;
  } while (a[i][j] != '+');
