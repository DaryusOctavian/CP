...
i64 solve(i64 pos, i64 left, vi64 &val, vi64 &wht, vec<vi64> &dp) {
  if (pos < 0) {
    return 0;
  }
  ...
  if (wht[pos] > left) {
    dp[pos][left] = solve(pos - 1, left, val, wht, dp);
    return dp[pos][left];
  }

  dp[pos][left] = max(val[pos] + solve(pos - 1, left - wht[pos], val, wht, dp),
                      solve(pos - 1, left, val, wht, dp));
  return dp[pos][left];
}

void reconstruct(i64 pos, i64 prev, vi64 &res, vec<vi64> &dp) {
  for (i64 i = 0; i <= c; i++) {
    if (dp[pos][i] == 0) {
      return;
    }
    if (dp[pos][i] != -1) {
      i64 crt = dp[pos + 1][prev] - dp[pos][i];
      for (i64 j = 0; j < n; j++) {
      }
    }
  }
...
int main() {
  ...
  while (cin >> c >> n) {
    vi64 val(n), wht(n);
    vec<vi64> dp(n + 1, vi64(c + 1, -1));
    for (i64 i : range(n)) {
      cin >> val[i] >> wht[i];
    }

    solve(n - 1, c, val, wht, dp);

    vi64 res;
    reconstruct(n - 2, c, res, dp);
  }
