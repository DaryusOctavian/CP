...
vi64 mxvec(vi64 a, vi64 b, vp64 &eep) {
  i64 x = 0, y = 0;
  for (auto &e : a) {
    x += eep[e].ft;
  }
...
vi64 solve(i64 pos, i64 left, vp64 &eep, vec<vec<vi64>> &memo) {
  if (pos < 0) {
    vi64 nig;
    return nig;
  }
  if (!memo[pos][left].empty()) {
    return memo[pos][left];
  }
  if (eep[pos].sd > left) {
    memo[pos][left] = solve(pos - 1, left, eep, memo);
    return memo[pos][left];
  }

  vi64 epus1 = solve(pos - 1, left - eep[pos].sd, eep, memo);
  epus1.psb(pos);
  vi64 epus2 = solve(pos - 1, left, eep, memo);
  memo[pos][left] = mxvec(epus1, epus2, eep);
  return memo[pos][left];
}
...
int main() {
  ...
  while (cin >> c >> n) {
    ...
    vec<vec<vi64>> memo(n + 1, vec<vi64>(c + 1));

    vi64 res = solve(n - 1, c, e, memo);
