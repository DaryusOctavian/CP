...
void coclender(vec<vec<char>> &v, i64 n, i64 m) {
  p64 res = {-1, -1};
  i64 rs = 0;
  for (i64 i = 0; i < n; i++) {
    for (i64 j = 0; j < m; j++) {
      if (v[i][j] == '.') {
        i64 crt = 0;
        for (i64 e1 = -1; e1 < 2; e1++) {
          for (i64 e2 = -1; e2 < 2; e2++) {
            if (!(e1 == 0 && e2 == 0)) {
              i64 posx = i + e1;
              i64 posy = j + e2;
          ...
          for (i64 e2 = -1; e2 < 2; e2++) {
            if (!(e1 == 0 && e2 == 0)) {
              ...
              if (posx >= 0 && posx < n && posy >= 0 && posy < m &&
                  v[posx][posy] == 'o') {
                i64 dev1 = i * m + j;
                i64 dev2 = posx * m + posy;
                if (find(col(g[dev1]), dev2) == g[dev1].end()) {
                  g[dev1].psb(dev2);
                }
                if (find(col(g[dev2]), dev1) == g[dev2].end()) {
                  g[dev2].psb(dev1);
                }
              }
            }
