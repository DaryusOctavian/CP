...
int main() {
  ...
  for (i64 i = n - 1; i >= 0; i--) {
    ...
    if (lasts.ft != -1 && lasts.sd != -1) {
      break;
    }
  }
  p64 mainLeaders = {-1, -1};
  for (i64 i : range(n)) {
    if (mainLeaders.ft == -1 && s[i] == 'G') {
      mainLeaders.ft = i + 1;
    }
    if (mainLeaders.sd == -1 && s[i] == 'H') {
      mainLeaders.sd = i + 1;
    }

    if (mainLeaders.ft != -1 && mainLeaders.sd != -1) {
      break;
    }
  }
  ...
  for (i64 i : range(n)) {
    i64 crt, eep = i + 1;
    cin >> crt;
    if (eep > mainLeaders.ft and eep > mainLeaders.sd) {
      continue;
    }
    if (s[i] == 'G') {
      if (eep == mainLeaders.ft and crt >= lasts.ft) {
        gs++;
      } else if (eep >= mainLeaders.ft and crt >= mainLeaders.sd) {
        gs++;
      }
    } else if (s[i] == 'H') {
      if (eep == mainLeaders.sd and crt >= lasts.sd) {
        hs++;
      } else if (eep >= mainLeaders.sd and crt >= mainLeaders.ft) {
        hs++;
      }
    }
  }
