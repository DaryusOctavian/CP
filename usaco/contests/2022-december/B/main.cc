...
i64 getmaxind(vi64 &v) {
  i64 res = 0;
  for (i64 i : range(sz(v) - 1)) {
    if (v[i] > v[res]) {
      res = i;
    }
  }
...
void reduce(i64 pos, vi64 &v, i64 rg) {
  for (i64 i : range(pos - rg, pos + rg + 1)) {
    i64 crt;
    if (i < 0) {
      crt = 0;
    } else if (i >= sz(v)) {
  ...
  for (i64 i : range(sz(v))) {
    if (pos + rg + i >= sz(v) && pos - rg - i < 0) {
      break;
    }

    if ((pos + rg + i) < sz(v) && v[pos + rg + i] > i) {
      v[pos + rg + i] = i;
    }
    if ((pos - rg - i) >= 0 && v[pos - rg - i] > i) {
      v[pos - rg - i] = i;
    }
  }
}

void genprefix(vi64 &v, vi64 &temp, i64 rg) {
  for (i64 i : range((i64)1, sz(v) - 1)) {
    v[i] = temp[min(i + rg, sz(v) - 1)] - temp[max(i - rg - 1, (i64)0)];
  }
}
...
int main() {
  ...
  while (tc--) {
    ...
    preh.front() = 0;
    for (i64 i : range(n)) {
      preg[i + 1] = preg[i] + (v[i] == 'G' ? 1 : 0);
      preh[i + 1] = preh[i] + (v[i] == 'H' ? 1 : 0);
    }
    preg.back() = preg[sz(preg) - 2];
    preh.back() = preh[sz(preh) - 2];
    ...
    vi64 prefix(n + 2);
    genprefix(prefix, preg, rg);
    while (true) {
      i64 mxi = getmaxind(prefix);
      if (prefix[mxi] == 0) {
        break;
      }
      res[mxi - 1] = 'G';
      nem++;
      reduce(mxi, prefix, rg);
    }

    genprefix(prefix, preh, rg);
    while (true) {
      i64 mxi = getmaxind(prefix);
      if (prefix[mxi] == 0) {
        break;
