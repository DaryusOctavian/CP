...
i64 computedeletes(vi64 gs, vi64 ls, i64 start) {
  for (i64 i = 0; i < start; i++) {
    gs.ppb();
  }

  while (!(ls.empty() || gs.empty()) && gs.back() > ls.back()) {
    ls.ppb();
    start++;
  }
...
int main() {
  ...
  srt(gs);
  sort(bg(ls), ed(ls), greater<i64>());

  i64 res = 1e18;
  for (i64 i = 0; i < sz(gs); i++) {
    res = min(computedeletes(gs, ls, i), res);
  }
