...
i64 fact(i64 n) {
  ...
  return memo[n] = (((n % MOD) * (fact(n - 1) % MOD)) % MOD);
}

i64 lpow(i64 n, i64 k) {
  if (k == 0) {
    return 1;
  }
  if (k == 1) {
  ...
  if (k % 2 == 0) {
    ...
    i64 ans = lpow(n, k / 2);
    return ((((((n) % MOD) * (ans % MOD)) % MOD) * (ans % MOD)) % MOD);
  }
}

i64 ifact(i64 n) {
  i64 x = fact(n);
  return (lpow(x, MOD - 2)) % MOD;
}
...
i64 comb(i64 n, i64 k) {
  ...
  return (((fact(n) % MOD) * (ifact(n - k) % MOD)) % MOD * (ifact(k) % MOD)) %
         MOD;
}
